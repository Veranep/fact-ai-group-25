...
from ValueFunctionNoLSTM import NeuralNetworkBased
        ...
        for day in TRAINING_DAYS:
            ...
            if (envt.num_days_trained % VALID_FREQ == VALID_FREQ - 1):
                ...
                if (isinstance(value_function, NeuralNetworkBased)):
                    if (test_score > max_test_score or (envt.num_days_trained % SAVE_FREQ) == (SAVE_FREQ - 1)) and value_function.trainer.model:
                        value_function.trainer.save_checkpoint('{}_{}agent_{}capacity_{}delay_{}interval_{}_{}.h5'.format(type(value_function).__name__, num_agents, CAPACITY, PICKUP_DELAY, DECISION_INTERVAL, envt.num_days_trained, test_score))
