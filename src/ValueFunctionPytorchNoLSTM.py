...
class PathModel(pl.LightningModule):
    ...
    def __init__(self, num_inputs, num_delay, data_dir):
        ...
        self.act_fn1 = nn.ELU()
        self.linear2 = nn.Linear(self.cap*100+self.cap+100+2, 300)
        self.act_fn2 = nn.ELU()
    ...
    def forward(self, x):
        ...
        delay = x[:,self.cap: 2*self.cap].float() #Bx9
        current_time = x[:,2*self.cap:2*self.cap+1].float()
        other_agents = x[:,2*self.cap+1:2*self.cap+2].float() #Bx1
        num_requests = x[:,2*self.cap+2:].float() #Bx1
        path_location_embed =  self.embed(path_location) #Bx9x100
        current_time = self.linear1(current_time)
        current_time = self.act_fn1(current_time) #Bx100
        state_embed = torch.hstack([torch.reshape(path_location_embed,(delay.shape[0], -1)), delay, current_time, other_agents, num_requests])
